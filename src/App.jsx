...
// Cache for keeping page states
const pageCache = new Map();

// Component to keep pages alive
function KeepAliveRoute({ children, cacheKey }) {
  ...
  }, [cacheKey, children, location]);
  
  // Restore scroll position when coming back
  useEffect(() => {
    const cached = pageCache.get(cacheKey);
    if (cached && cached.scrollPosition) {
      setTimeout(() => {
...
function App() {
  ...
  useEffect(() => {
    ...
    // Save state for the current page before leaving
    return () => {
      ...
      <AuthProvider>
        <WishlistProvider>
          <CartProvider>
            ...
            <Navbar />
            <Routes>
              {/* Public Routes - Wrapped with KeepAliveRoute for caching */}
              <Route path="/" element={
                <KeepAliveRoute cacheKey="/">
                  <Home />
                </KeepAliveRoute>
              } />
